...
std::vector<unsigned char> encrypt_AES_OFB(std::vector<unsigned char> plaintext, std::vector<unsigned char> key_vector, std::vector<unsigned char> iv_vector) {
...
std::vector<unsigned char> decrypt_AES_OFB(std::vector<unsigned char> ciphertext, std::vector<unsigned char> key_vector, std::vector<unsigned char> iv_vector) {
...
int main(int argc, char* argv[]) {
	...
	cout << "Using AES-" << (8*AES_SIZE) << " in OFB mode" << endl << endl;
	...
	string message_string = "This is a message to be encrypted with AES in OFB mode.";
	...
	// Encrypted message
	std::vector<unsigned char> encrypted_message = encrypt_AES_OFB(message, key, iv);
	cout << " ~ Encrypted Message (hex)" << endl;
	for (auto val : encrypted_message) printf("%.2x", val);
	cout << endl;

	// Decrypted message
	std::vector<unsigned char> decrypted_message = decrypt_AES_OFB(encrypted_message, key, iv);
